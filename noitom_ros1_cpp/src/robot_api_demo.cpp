...
json readJsonFile(const std::string& filename) {
    std::string package_path = ros::package::getPath("noitom_ros1_cpp");
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "json_to_joint_state_publisher");
  ros::NodeHandle nh;
  publisher_ = nh.advertise<sensor_msgs::JointState>("/joint_states", 10);
  ros::Timer timer_ = nh.createTimer(ros::Duration(0.1), timer_callback);
  ros::spin();
